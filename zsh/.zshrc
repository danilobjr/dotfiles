...
alias gb='git br' # branch
alias gcl='git cl' # clone
...
ni()  { "$PKG_MANAGER" install    "$@" }
nr()  { "$PKG_MANAGER" run        "$@" }
ns()  { "$PKG_MANAGER" run start  "$@" }
nrb() { "$PKG_MANAGER" run build  "$@" }
nrd() { "$PKG_MANAGER" run dev    "$@" }
nrl() { "$PKG_MANAGER" run lint   "$@" }
nrt() { "$PKG_MANAGER" run test   "$@" }

na()  { "$PKG_MANAGER" add -E     "$@" }
nad() { "$PKG_MANAGER" add -E -D  "$@" }
nu()  { "$PKG_MANAGER" remove     "$@" }
...
# node pkg manager detection
[[ ! -f $DOTFILES/zsh/powerlevel10k/config/hook_detect_node_pkg_manager.zsh ]] || source $DOTFILES/zsh/powerlevel10k/config/hook_detect_node_pkg_manager.zsh
# asdf
